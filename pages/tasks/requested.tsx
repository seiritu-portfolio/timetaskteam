...
// * hooks
import { useTasksRequestedWithUser } from '@service/taskQueries';
...
const TasksType = () => {
  ...
  const tasksRequestedResult = useTasksRequestedWithUser(
  ...
  useEffect(() => {
    if (tasksRequestedResult.isSuccess) {
      const processedData = getClassified(tasksRequestedResult.data, sortType);
      setResultData(processedData);
    }
  }, [
    tasksRequestedResult.isSuccess,
    tasksRequestedResult.data,
    dispatch,
    sortType,
  ]);
        ...
        <div className="mt-12px px-24px relative ">
          ...
          {resultData.map((list, index) => (
            <TaskClassifiedSection
              ...
              onHeader={onTaskAdd}
              setSelectedList={() => {}}
              droppableId={`tasks-requested-${sortType}-${list.key}-${currentCodisplayUserID}`}
            />
          ))}
        </div>
